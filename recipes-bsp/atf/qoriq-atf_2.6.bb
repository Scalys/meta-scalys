require recipes-bsp/atf/qoriq-atf-${PV}.inc
...
SRC_URI += "git://github.com/ARMmbed/mbedtls;protocol=https;nobranch=1;destsuffix=git/mbedtls;name=mbedtls \
    ...
    file://0001-Added-support-for-qls1046a4gb.patch \
"
...
PLATFORM:qls1046a-4gb = "qls1046a4gb"
PLATFORM:qls1046-4gb-dkb = "qls1046a4gb"
PLATFORM:qls1046-4gb-dk = "qls1046a4gb"

RCW_FOLDER ?= "${MACHINE}"
RCW_FOLDER:ls1088ardb-pb = "ls1088ardb"
RCW_FOLDER:lx2160ardb = "lx2160ardb_rev2"
RCW_FOLDER:lx2160ardb-rev2 = "lx2160ardb_rev2"
RCW_FOLDER:qls1046-4gb-dkb = "qls1046a-4gb"
RCW_FOLDER:qls1046-4gb-dk = "qls1046a-4gb"
...
do_compile() {
    ...
    for d in ${BOOTTYPE}; do
        ...
        nor)
            ...
            UBOOT_BINARY="${DEPLOY_DIR_IMAGE}/u-boot.bin-nor"
            ;;
        nor_xfi)
            rcwimg="${RCWNORXFI}${RCW_SUFFIX}"
            UBOOT_BINARY="${DEPLOY_DIR_IMAGE}/u-boot.bin-nor_xfi"
            ;;
        nand)
        ...
        sd)
            ...
            UBOOT_BINARY="${DEPLOY_DIR_IMAGE}/u-boot.bin-sdhc"
            ;;
        sd_xfi)
            rcwimg="${RCWSDXFI}${RCW_SUFFIX}"
            UBOOT_BINARY="${DEPLOY_DIR_IMAGE}/u-boot.bin-sdhc_xfi"
            ;;
        emmc)
        ...
        if [ -f ${DEPLOY_DIR_IMAGE}/rcw/${RCW_FOLDER}/$rcwimg ]; then
            ...
            oe_runmake V=1 all fip pbl ${FIP_DDR} PLAT=${PLATFORM} BOOT_MODE=${d%%_*} RCW=${DEPLOY_DIR_IMAGE}/rcw/${RCW_FOLDER}/${rcwimg} BL33=${UBOOT_BINARY}
            cp build/${PLATFORM}/release/bl2_${d%%_*}${SECURE_EXTENTION}.pbl bl2_${d}${SECURE_EXTENTION}.pbl
            cp build/${PLATFORM}/release/fip.bin fip_uboot_${d}${SECURE_EXTENTION}.bin
